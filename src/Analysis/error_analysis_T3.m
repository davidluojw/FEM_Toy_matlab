function  model =  error_analysis_T3(model)
    ...
    for ee = 1 : model.nel
        ...
        for ll = 1:model.nint
            ...
            for aa = 1:model.nen
                ...
                x_l = x_l + x_ele(aa) * Area_ShapeBasisN_2d(model.degree, aa, xi(ll), eta(ll), zeta(ll));
                y_l = y_l + y_ele(aa) * Area_ShapeBasisN_2d(model.degree, aa, xi(ll), eta(ll), zeta(ll));
                ux_l = ux_l + ux_ele(aa) * Area_ShapeBasisN_2d(model.degree, aa, xi(ll), eta(ll), zeta(ll));
                uy_l = uy_l + uy_ele(aa) * Area_ShapeBasisN_2d(model.degree, aa, xi(ll), eta(ll), zeta(ll));

                [Na_xi, Na_eta] = Area_ShapeBasisN_Grad_2d(model.degree, aa, xi(ll), eta(ll));
