function  model =  elastodynamics_error_analysis_Q4(model)

    % store the error for each time step
    model.errorL2_tt = zeros(1, model.nts);
    model.errorH1_tt = zeros(1, model.nts);
    ...
    for tt = 2:model.nts
        ...
        for ee = 1 : model.nel
            ...
            for ll = 1:model.nint
                ...
                for aa = 1:model.nen
                    ...
                    x_l = x_l + x_ele(aa) * Quad_ShapeBasisN_2d(model.deg_xi, model.deg_eta, aa, xi(ll), eta(ll));
                    y_l = y_l + y_ele(aa) * Quad_ShapeBasisN_2d(model.deg_xi, model.deg_eta, aa, xi(ll), eta(ll));
                    ux_l = ux_l + ux_ele(aa) * Quad_ShapeBasisN_2d(model.deg_xi, model.deg_eta, aa, xi(ll), eta(ll));
                    uy_l = uy_l + uy_ele(aa) * Quad_ShapeBasisN_2d(model.deg_xi, model.deg_eta, aa, xi(ll), eta(ll));

                    [Na_xi, Na_eta] = Quad_ShapeBasisN_Grad_2d(model.deg_xi, model.deg_eta, aa, xi(ll), eta(ll));
                ...
                model.errorL2_x = model.errorL2_x + weight(ll) * detJ * (ux_l - model.exact_ux(x_l, y_l, cur_t))^2;
                model.errorL2_y = model.errorL2_y + weight(ll) * detJ * (uy_l - model.exact_uy(x_l, y_l, cur_t))^2;

                model.errorH1_x = model.errorH1_x + weight(ll) * detJ * ( ( ux_l_x- model.exact_ux_x(x_l,y_l,cur_t))^2 + ( ux_l_y - model.exact_ux_y(x_l,y_l,cur_t))^2 );
                model.errorH1_y = model.errorH1_y + weight(ll) * detJ * ( ( uy_l_x- model.exact_uy_x(x_l,y_l,cur_t))^2 + ( uy_l_y - model.exact_uy_y(x_l,y_l,cur_t))^2 );

                bottomL2_x = bottomL2_x + weight(ll) * detJ * model.exact_ux(x_l, y_l, cur_t)^2;
                bottomL2_y = bottomL2_y + weight(ll) * detJ * model.exact_uy(x_l, y_l, cur_t)^2;

                bottomH1_x = bottomH1_x + weight(ll) * detJ * ( model.exact_ux_x(x_l,y_l,cur_t)^2 + model.exact_ux_y(x_l,y_l,cur_t)^2 );
                bottomH1_y = bottomH1_y + weight(ll) * detJ * ( model.exact_uy_x(x_l,y_l,cur_t)^2 + model.exact_uy_y(x_l,y_l,cur_t)^2 );
            end
        end
        ...
        model.errorL2_tt(tt) = model.errorL2;
        model.errorH1_tt(tt) = model.errorH1;
    end
