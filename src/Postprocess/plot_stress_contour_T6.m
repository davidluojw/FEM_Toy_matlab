function plot_stress_contour_T6(model, type)
    ...
    for i = 1:model.nel
        ...
        node_idsreo = [node_ids(1), node_ids(4), node_ids(2), node_ids(5), node_ids(3), node_ids(6)];  % reorder the index
        x_nodes = model.nodes(node_idsreo, 1);
        y_nodes = model.nodes(node_idsreo, 2);
        
        XX = [x_nodes(1), x_nodes(2), x_nodes(3), x_nodes(4), x_nodes(5), x_nodes(6), x_nodes(1)];
        YY = [y_nodes(1), y_nodes(2), y_nodes(3), y_nodes(4), y_nodes(5), y_nodes(6),  y_nodes(1)];
        ...
        if type == 1
            dd = [sxx(1), sxx(2), sxx(3), sxx(4), sxx(5), sxx(6), sxx(1)];
        elseif type == 2
            dd = [syy(1), syy(2), syy(3), syy(4), syy(5), syy(6), syy(1)];
        elseif type == 3
            dd = [sxy(1), sxy(2), sxy(3), sxy(4), sxy(5), sxy(6), sxy(1)];
        elseif type == 4
            dd = [mises(1), mises(2), mises(3), mises(4), mises(5), mises(6), mises(1)];
        end
        patch(XX,YY,dd);
        hold on;
    end
