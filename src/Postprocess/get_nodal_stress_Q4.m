function model = get_nodal_stress(model, ee)
    ...
    for ee = 1:model.nel
        ...
        for ii = 1 : model.nen
            ...
            [N1_xi, N1_eta] = Quad_ShapeBasisN_Grad_2d(model.deg_xi, model.deg_eta, 1, xi(ii), eta(ii));
            [N2_xi, N2_eta] = Quad_ShapeBasisN_Grad_2d(model.deg_xi, model.deg_eta, 2, xi(ii), eta(ii));
            [N3_xi, N3_eta] = Quad_ShapeBasisN_Grad_2d(model.deg_xi, model.deg_eta, 3, xi(ii), eta(ii));
            [N4_xi, N4_eta] = Quad_ShapeBasisN_Grad_2d(model.deg_xi, model.deg_eta, 4, xi(ii), eta(ii));
